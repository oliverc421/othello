		...
		for j in range (-1,2):
			try:
				if board[(coordinate[0]+i)][(coordinate[1]+j)] == opponent:	# i want to capture this direction to check if there is a same colour counter further down
					for k in range (1,8):
						try:
							if board[(coordinate[0]+k*i)][(coordinate[1]+k*j)] != opponent: # timesing k by i and j means it goes in the 														specfic direction it needs to, if the target counter is 													to the left (coordinate(0) - 1) then continuing to minus 														is what i want to do
								if board[(coordinate[0]+k*i)][(coordinate[1]+k*j)] == 'None ':
									break
								else:
									return True
						except: 
